...
ibkr_category = os.environ.get("IBKR_CATEGORY").split(",")
ghost_currency = os.environ.get("GHOST_CURRENCY", "USD").split(",")
operations = os.environ.get("OPERATION", SYNCIBKR).split(",")

if __name__ == '__main__':
    for i in range(len(operations)):
        ghost = SyncIBKR(ghost_hosts[i], ibkr_tokens[i], ibkr_queries[i], ghost_tokens[i], ghost_currency[i], ibkr_category[i])
